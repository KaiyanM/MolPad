...
gDashboard(cheesedata,cluschee,pathchee,networkchee,dashboardtitle = "Test",id_colname = c("GO_ID"),id_type = c("GO"))
...
```{r}
set.seed(123)
sample <- sample(1:10000,100)
sample

test_data <- cheese[sample,c(1:5)]
test_annotations <- annotations[sample,c(1:3,4:7)]


test_data <- test_data |>
  dplyr::mutate(test_annotations["kingdom"]) |>
  dplyr::rename(type=kingdom)

test_data_processed <- test_data |>
  pre_process()

test_clus_res <- gClusters(test_data_processed,ncluster = 10,elbow.max=15)
test_net_res <- gNetwork(test_clus_res,ntop = 3)
test_net_res
test_net_res <- na.omit(test_net_res)

test_annotations_processed <- gAnnotation(test_annotations,phylum,class)
test_annotations_processed
test_clus_res
gDashboard(test_data_processed,test_clus_res,test_annotations_processed,test_net_res,dashboardtitle = "Test",id_colname = c("GO_ID","web_id"),id_type = c("GO","KEGG"))
#99-pathway error
```
