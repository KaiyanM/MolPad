...
#' @name pre_process
#'
#' @param data A data.frame to describe each feature in one row. The data should contain variables \code{ID}, \code{value on time_1}, ..., \code{value on time_k}, and \code{type} for extracting patterns across the time. If multiple data.frame with the above format needs to be analyzed, the user could also put a list of data.frame into this argument. In this case, variable \code{type} is not required and will be generated by the next argument \code{typenameList}.
