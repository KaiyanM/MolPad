...
#'
#' @description The `pre_process()` function aids in processing data inputs and automatically establishes a standardized format for future use. 
...
#' @name pre_process
#'
#' @param data A data.frame to describe each feature in one row. 
#' The data should contain variables \code{ID} \code{value on time_1}, ..., \code{value on time_k}, and \code{type} for extracting patterns across the time. 
#' Note that the initial and last column must be exactly `ID` and `type`.
